...
def classify_topic(text: str) -> Dict[str, Any]:
    ...
    if not results:
        return {"label": "news_&_social_concern", "score": 0.5, "model": "gliner2"}

    # Return highest scoring topic
    best = max(results, key=lambda x: x.get("score", 0))
    return {
        "label": best.get("label", "news_&_social_concern"),
        "score": round(best.get("score", 0.5), 4),
        "model": "gliner2"
    }
...
def analyze_article(title: str, description: str = None, content: str = None,
                   include_embedding: bool = True) -> Dict[str, Any]:
    """
    ...
    if content:
        combined += " " + content[:500]  # First 500 chars of content
...
def batch_analyze_articles(articles: List[Dict[str, str]],
                          include_embedding: bool = True) -> List[Dict[str, Any]]:
    """
    ...
    for article in articles:
        ...
        if article.get("content"):
            combined += " " + article["content"][:500]
        combined_texts.append(combined)

    # Get embeddings in batch if needed
    embeddings = get_embedding_batch(combined_texts) if include_embedding else [None] * len(articles)

    # Analyze each article
    results = []
    for combined_text, embedding in zip(combined_texts, embeddings):
        result = {
